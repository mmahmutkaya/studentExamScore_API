exports = async function (request, response) {
  ...
  AUTH_CHECK: try {
    ...
    return ({hata:true,hataYeri:"FONK // dersSave",hataMesaj:"Şube kaydetmeye yetkiniz bulunmuyor."})
  ...
  let is_branchName_Violation = false
  let violation_branchName_ExcelRows = []
  ...
  let is_branchName_Exist = false
  let exist_branchName_ExcelRows = []
  ...
  let is_branchName_Exist_inCame = false
  let exist_branchName_ExcelRows_inCame = []
  ...
  try {
    ...
    await cameItems.map(item => {
      ...
      if(item.branchName.length < 3) {
        is_branchName_Violation = true
        violation_branchName_ExcelRows.push(item.siraNo)
      }
      ...
      if(branchArray.find(x=> x.name == item.branchName)) {
        is_branchName_Exist = true
        exist_branchName_ExcelRows.push(item.siraNo)
      }
      ...
      loopObj = null
      loopObj = cameItems_Add.find(x=> x.branchName == item.branchName) 
      if(loopObj) {
        is_branchName_Exist_inCame = true
        exist_branchName_ExcelRows_inCame.push(loopObj.siraNo)
        exist_branchName_ExcelRows_inCame.push(item.siraNo)
        exist_branchName_ExcelRows_inCame.push("---")
      }
    ...
    if (is_branchName_Violation) {
      satirNumaralariArray = violation_branchName_ExcelRows
      satirNumaralariArray.length > 1 ? currentCondition = "kayıtlardaki" : currentCondition = "kayıttaki"
      return ({hata:true,hataYeri:"FONK // dersSave // MONGO-5",hataMesaj: satirNumaralariArray +  " numaralı " + currentCondition + " \"şube isim\" bilgisi kontrol edilmeli, 3 karakterden az."});
    }
    ...
    if (is_branchName_Exist) {
      satirNumaralariArray = exist_branchName_ExcelRows
      satirNumaralariArray.length > 1 ? currentCondition = "kayıtlardaki" : currentCondition = "kayıttaki"
      return ({hata:true,hataYeri:"FONK // dersSave // MONGO-5",hataMesaj: satirNumaralariArray +  " numaralı " + currentCondition + " \"şube adı\" sistemde mevcut."});
    }
    ...
    if (is_branchName_Exist_inCame) {
      satirNumaralariArray = exist_branchName_ExcelRows_inCame
      satirNumaralariArray.length > 1 ? currentCondition = "kayıtlardaki" : currentCondition = "kayıttaki"
      return ({hata:true,hataYeri:"FONK // dersSave // MONGO-5",hataMesaj: satirNumaralariArray +  " numaralı " + currentCondition + " \"şube adı\" bilgilerinde mükerrerlik var, kontrol ediniz."});
    }
    ...
    await collectionBranchs.bulkWrite(bulk, { ordered: false });
