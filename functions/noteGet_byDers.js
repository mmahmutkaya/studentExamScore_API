exports = async function (request, response) {
  ...
  try {
    ...
    ///
    lesson = await collectionLessons.findOne({dersNo})
    hataText = "\"gelen istekteki \"ders\" sistemde bulunamadı\""
    if(!lesson) return ({hata:true,hataYeri:"FONK // noteGet",hataMesaj:"Program yöneticisi ile iletişime geçmeniz gerekmektedir, (" + hataText +")"})
  ...
  AUTH_CHECK: try {
    ...
    if(user.ogretmenNo == lesson.ogretmenNo) {
      break AUTH_CHECK
    }
